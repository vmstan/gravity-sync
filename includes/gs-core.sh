...
function ph_type {
    ...
    then
        PH_EXEC="${DOCKER_BIN} exec ${CON_NAME} pihole"
    elif [ "$PH_IN_TYPE" == "podman" ]
    then
        PH_EXEC="${PODMAN_BIN} exec ${CON_NAME} pihole"
    fi
    ...
    then
        RH_EXEC="${ROCKER_BIN} exec ${RON_NAME} pihole"
    elif [ "$RH_IN_TYPE" == "podman" ]
    then
        RH_EXEC="${RODMAN_BIN} exec ${RON_NAME} pihole"
    fi
}
